...
3.  Your new `move` directory should now resemble:
    ...
    ![move-directory](/assets/docs/v1.0.1/build-e2e-dapp-img-1.png)
...
1. In our `move` directory, run `aptos init --network devnet`. Press enter when prompted.
   ...
   <Callout type="info">
     You just created a new account on the Aptos (dev) network! Yay! You can see
     it by going to the [Aptos
     Explorer](https://explorer.aptoslabs.com/?network=devnet) Devnet network
     view, pasting the `account` address value from your configuration file into
     the search field, and clicking on the dropdown option!
   </Callout>
...
<Callout type="info">
  Creating an event is not mandatory yet useful if dapps/users want to monitor
  data, such as how many people create a new task, using the [Aptos
  Indexer](../../indexer.mdx).
</Callout>
